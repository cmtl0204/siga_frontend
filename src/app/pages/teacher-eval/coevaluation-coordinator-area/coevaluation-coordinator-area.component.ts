...
@Component({
  selector: 'app-coevaluation-coordinator-area',
  templateUrl: './coevaluation-coordinator-area.component.html',
  styleUrls: ['./coevaluation-coordinator-area.component.css']
})



export class CoevaluationCoordinatorAreaComponent implements OnInit {
  ...
  constructor(public messageService: MessageService,
    ...
    this.paginator = { current_page: 1, per_page: 1 };
    this.questions = [];
  ...
  onTestWebService() {
    this.teacherEvalService.getEvaluationArea(1).subscribe(result => {
  ...
  getCheckSelect() {
    ...
    this.teacherEvalService.postEvaluationArea(dataSave).subscribe(result => {
      console.log(result);
