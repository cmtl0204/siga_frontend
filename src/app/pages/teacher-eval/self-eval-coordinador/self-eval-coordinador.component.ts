...
@Component({
  selector: 'app-self-eval-coordinador',
  templateUrl: './self-eval-coordinador.component.html',
  styleUrls: ['./self-eval-coordinador.component.css']
})
export class SelfEvalCoordinadorComponent implements OnInit {
...
totalpreguntas: number;
 ...
 // navegacion de componnetes 
  home(){
   ...
   autovaluation(){
    this.router.navigate(['teacher-eval/coordinador-list-selfevaluation']);
   }
  ...
  onTestWebServicePairs() {
    this.teacherEvalService.getSelfevaluation(1).subscribe(result => {
  ...
  getCheckSelect() {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        if (isValidateRadio) {
          this.teacherEvalService.postSelfevaluation(this.id, data ).subscribe(result => {
            this.showSuccess();
          }, error => {
          });
        } else{
